...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto* rootFrame = new tsl::elm::OverlayFrame(packageName,
           !lastPackageHeader.empty() ? lastPackageHeader + "?Ryazhahand Script" : (packageVersion.empty() ? CAPITAL_ULTRAHAND_PROJECT_NAME + " Script" : packageVersion + " "+DIVIDER_SYMBOL+" " + CAPITAL_ULTRAHAND_PROJECT_NAME + " Script"),
           noClickableItems);
        list->disableCaching();
        rootFrame->setContent(list);
        if (showWidget)
