...
[![Latest Version](https://img.shields.io/github/v/release/ppkantorski/libryazhahand?label=latest&color=blue)](https://github.com/ppkantorski/libryazhahand/releases/latest)
[![Downloads](https://img.shields.io/github/downloads/ppkantorski/libryazhahand/total?color=6f42c1)](https://github.com/ppkantorski/libryazhahand/graphs/traffic)
[![GitHub issues](https://img.shields.io/github/issues/ppkantorski/libryazhahand?color=222222)](https://github.com/ppkantorski/libryazhahand/issues)
...
For easy importing and inclusion, try using `ryazhahand.mk`. These lines must go after your projects `SOURCES` and `INCLUDES` definitions.
```sh
...
## Build Examples
- [Ryzhand Overlay](https://github.com/ppkantorski/Ryzhand-Overlay)
...
Users can specify custom Ryzhand `theme.ini` and `wallpaper.rgba` files for the overlay to use located in your SD card's `/config/<OVERLAY_NAME>/` or `{UI_OVERRIDE_PATH}` directory.
...
Jsons will need to be named ISO 639-1 format (en, de, fr, es, etc...) and will only be used in accordance with the current language set in the Ryzhand Overlay `/config/ryazhahand/config.ini`.
...
### Ryzhand Overlay Widget

To add the Ryzhand Overlay widget to your `OverlayFrame`'s, add the following directive to your `Makefile`:
...
```cpp
tsl::initializeThemeVars(); // Initialize variables for ryazhahand themes
```
...
Contributions are welcome! If you have any ideas, suggestions, or bug reports, please raise an [issue](https://github.com/ppkantorski/libryazhahand/issues/new/choose), submit a [pull request](https://github.com/ppkantorski/libryazhahand/compare) or reach out to me directly on [GBATemp](https://gbatemp.net/threads/ryazhahand-overlay-the-fully-craft-able-overlay-executor.633560/).
