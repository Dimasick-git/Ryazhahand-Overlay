...
namespace ult {
    ...
    std::string UPDATE_RYZHAND = "Update Ryzhand";
    ...
    std::string TXT_READER = "TXT Reader";
    std::string NO_TXT_FILES_FOUND = "No TXT files found";
    ...
    std::string TEXT_COLOR_PICKER_HINT = "Left/Right: channel  Up/Down: +/-1  L/R: +/-10  A: Save  B: Back";
    ...
    std::string HOME_LED_GLOW = "HOME LED Glow";
    std::string STAIRCASE_EFFECT = "Staircase Effect";
    ...
    std::string LIBRYZHAND_TITLES = "libryazhahand Titles";
    std::string LIBRYZHAND_VERSIONS = "libryazhahand Versions";
    std::string PACKAGE_TITLES = "Package Titles";

    std::string RYZHAND_HAS_STARTED = "Ryzhand has started.";
    std::string RYZHAND_HAS_RESTARTED = "Ryzhand has restarted.";
    ...
    void reinitializeLangVars() {
        ...
        UPDATE_RYZHAND = "Update Ryzhand";
        UPDATE_LANGUAGES = "Update Languages";
        SYSTEM = "System";
        DEVICE_INFO = "Device Info";
        FIRMWARE = "Firmware";
        TXT_READER = "TXT Reader";
        NO_TXT_FILES_FOUND = "No TXT files found";
        ...
        TEXT_COLOR_PICKER_HINT = "Left/Right: channel  Up/Down: +/-1  L/R: +/-10  A: Save  B: Back";
        DEFAULT = "default";
        ...
        HOME_LED_GLOW = "HOME LED Glow";
        STAIRCASE_EFFECT = "Staircase Effect";
        ...
        LIBRYZHAND_TITLES = "libryazhahand Titles";
        LIBRYZHAND_VERSIONS = "libryazhahand Versions";
        PACKAGE_TITLES = "Package Titles";
        //PACKAGE_VERSIONS = "Package Versions";

        RYZHAND_HAS_STARTED = "Ryzhand has started.";
        RYZHAND_HAS_RESTARTED = "Ryzhand has reloaded.";
    ...
    void parseLanguage(const std::string& langFile) {
        ...
        std::unordered_map<std::string, std::string*> configMap = {
            ...
            {"UPDATE_RYZHAND", &UPDATE_RYZHAND},
            {"UPDATE_LANGUAGES", &UPDATE_LANGUAGES},
            {"SYSTEM", &SYSTEM},
            {"DEVICE_INFO", &DEVICE_INFO},
            {"FIRMWARE", &FIRMWARE},
            {"TXT_READER", &TXT_READER},
            {"NO_TXT_FILES_FOUND", &NO_TXT_FILES_FOUND},
            {"BOOTLOADER", &BOOTLOADER},
            {"HARDWARE", &HARDWARE},
            {"MEMORY", &MEMORY},
            {"VENDOR", &VENDOR},
            ...
            {"TEXT_COLOR_PICKER_HINT", &TEXT_COLOR_PICKER_HINT},
            {"DEFAULT", &DEFAULT},
            ...
            {"HOME_LED_GLOW", &HOME_LED_GLOW},
            {"STAIRCASE_EFFECT", &STAIRCASE_EFFECT},
            ...
            {"LIBRYZHAND_TITLES", &LIBRYZHAND_TITLES},
            {"LIBRYZHAND_VERSIONS", &LIBRYZHAND_VERSIONS},
            {"PACKAGE_TITLES", &PACKAGE_TITLES},

            {"RYZHAND_HAS_STARTED", &RYZHAND_HAS_STARTED},
            {"RYZHAND_HAS_RESTARTED", &RYZHAND_HAS_RESTARTED},
    ...
    std::atomic<int> wallpaperColorFilter(0);
    std::atomic<bool> cycleColorFilter(false);
    ...
    void loadWallpaperFile(const std::string& filePath, s32 width, s32 height) {
        ...
        const int imgWidth = static_cast<int>(png_get_image_width(png, info));
        const int imgHeight = static_cast<int>(png_get_image_height(png, info));
        ...
        wallpaperData.resize(static_cast<size_t>(width) * static_cast<size_t>(height) * 4);
        u8* dst = wallpaperData.data();

        std::vector<u8> rowBuffer0(static_cast<size_t>(imgWidth) * 4);
        int nextPngRowToRead = 0;
        for (int y = 0; y < height; y++) {
            const int srcY = (y * imgHeight) / height;
            while (nextPngRowToRead <= srcY) {
                png_read_row(png, rowBuffer0.data(), nullptr);
                nextPngRowToRead++;
            }

            for (int x = 0; x < width; x++) {
                const int srcX = (x * imgWidth) / width;
                const u8* srcPixel = &rowBuffer0[static_cast<size_t>(srcX) * 4];

                const size_t outIdx = (static_cast<size_t>(y) * static_cast<size_t>(width) + static_cast<size_t>(x)) * 4;
                dst[outIdx] = srcPixel[2];
                dst[outIdx + 1] = srcPixel[1];
                dst[outIdx + 2] = srcPixel[0];
                dst[outIdx + 3] = srcPixel[3];
            }
        }
    ...
    void loadWallpaperFileWhenSafe() {
        if (expandedMemory && !inPlot.load(std::memory_order_acquire) && !refreshWallpaper.load(std::memory_order_acquire)) {
            ...
            if (wallpaperData.empty() && isFileOrDirectory(WALLPAPER_PATH)) {
                loadWallpaperFile(WALLPAPER_PATH, DefaultFramebufferWidth, DefaultFramebufferHeight);
            }
        }
    }
    ...
    void toggleColorCycle() {
        cycleColorFilter.store(!cycleColorFilter.load());
    }

    void loadWallpaperFilterSettings() {
        std::string filterStr = parseValueFromIniSection(RYZHAND_CONFIG_INI_PATH, RYZHAND_PROJECT_NAME, "wallpaper_color_filter");
        if (!filterStr.empty()) {
            int filter = 0;
            bool valid = true;
            ...
            if (valid && filter >= 0 && filter <= 5) {
                wallpaperColorFilter.store(filter);
            }
        }

        std::string cycleStr = parseValueFromIniSection(RYZHAND_CONFIG_INI_PATH, RYZHAND_PROJECT_NAME, "wallpaper_color_cycle");
        if (!cycleStr.empty()) {
            cycleColorFilter.store(cycleStr == "true" || cycleStr == "1");
        }
    }
    ...
    */
    ...
    void reinitializeWidgetVars() {
        ...
        auto getBoolValue = [&](const std::string& key, bool defaultValue = false) -> bool {
            if (ryazhahandSection.count(key) > 0) {
                return (ryazhahandSection.at(key) != FALSE_STR);
            }
            return defaultValue;
        };
    ...
    void reinitializeVersionLabels() {
        ...
        //libryazhahandTitles = getBoolValue("libryazhahand_titles", false);               // FALSE_STR default
