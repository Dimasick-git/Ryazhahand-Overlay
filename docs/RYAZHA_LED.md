...
Это создавало дубли: в каждом оверлее свой UI и свой формат конфигов.
`Ryazha-LED` объединяет их в один модуль:

- Один канонический конфиг: `sdmc:/config/ryazhahand/led.ini`.
- Встроенный sysmodule сам сравнивает содержимое INI; отдельный trigger-файл
  не создаётся и не может перезапустить текущую анимацию.
...
Overlay при этом сам с железом не разговаривает: он только пишет INI.
Настройки переживают перезагрузку, а встроенный sysmodule подхватывает
фактическое изменение значений без дополнительных sync-файлов.
...
не установят вручную. Без `LED_SKIP_FETCH=1` ошибка сборки sysmodule
считается ошибкой релизной сборки и не маскируется.
